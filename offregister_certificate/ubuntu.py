...
def self_signed0(*args, **kwargs):
    ...
    # ${VAR%/*}
    run_cmd('mkdir -p `basename {keyout}` `basename {certout}`'.format(keyout=kwargs['SSL_KEYOUT'],
                                                                       certout=kwargs['SSL_CERTOUT']))

    run_cmd('openssl req {}'.format(' '.join(
        ('-new', '-newkey ec', '-pkeyopt ec_paramgen_curve:prime256v1', '-days 365',
         '-nodes', '-x509',
    ...
    return 'generated: {keyout} {certout}'.format(keyout=kwargs['SSL_KEYOUT'], certout=kwargs['SSL_CERTOUT'])
